  ...
  <article>
    ...
    <div v-for="palette in items" :key="palette.name">
      ...
      <div class="flex gap-2 p-1 overflow-y-auto">
        <div class="block w-26" v-for="(color, index) in palette.colors" :key="color.name">
          <button
            ...
            :style="{ 'background-color': color.value, 'color': index > 2? 'white': 'black' }"
          >
    ...
    <div class="prose">
      <p>The color palette is delivered as a "theme" for <strong>tailwind/windicss</strong> and is also accesible in code by including {colors} from "@vejron/ue-design" </p>
    </div>
